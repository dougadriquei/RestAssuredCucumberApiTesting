...
Feature: UC01 - PostPixPayments
  ...
  Scenario: UC01.02 - The Application must validate if it is a invalid payment
    Given I want perform GET operation for "/accounts/62f87af90ba501353df77b95"
    And I perform POST operations for "/pix/codes" with body as
      | encoded_value |
      | 00020126330014BR.GOV.BCB.PIX011170326165690520400005303986540510.005802BR5911Builders 016002SP62150511PAGAMENTO0163044312 |
  ...
  Scenario: UC01.04 - The Application must validate if it is a not found with conciliation_id invalid
    ...
    #TODO - Deveria retornar 201, mas está retornando status code 200
  Scenario: UC01.05 - The Application must validate if it is a invalid payment without sender bank
  ...
  Scenario: UC01.06 -  The Application must validate if it is a invalid payment without sender
    ...
    #TODO - Erro 500
  Scenario: UC01.07 - The Application must validate if it is a invalid payment without recipient bank
    Given I want perform GET operation for "/accounts/62f87af90ba501353df77b95"
  ...
  Scenario: UC01.08 - The Application must validate if it is a invalid payment without recipient@@@
    Given I want perform GET operation for "/accounts/62f87af90ba501353df77b95"
  ...
  Scenario: UC01.09 - The Application must validate if it is a invalid payment with negative amount
    Given I want perform GET operation for "/accounts/62f87af90ba501353df77b95"
  ...
  Scenario: UC01.10 - The Application must validate if it is a invalid payment without amount@@@@
    Given I want perform GET operation for "/accounts/62f87af90ba501353df77b95"
  ...
  Scenario: UC01.11 - The Application must validate if it is a invalid payment with document sender invalid@@@
  ...
  Scenario: UC01.12 - The Application must validate if it is a invalid payment when Sender and Recipient are the same
    Given I want perform GET operation for "/accounts/62f87af90ba501353df77b95"
    ...
    And I should see the bad request as "invalid_request" with errors document sender as "recipient cannot be the same as the sender" and with status code 400
     ...
     #TODO - Deveria retornar 201, mas está retornando status code 200
  Scenario: UC01.13 - The Application must validate if it is a valid payment QR CODE 02
    Given I want perform GET operation for "/accounts/62f87af90ba501353df77b95"
    And I perform POST operations for "/pix/codes" with body as
      | encoded_value |
    ...
    And I should see the body has authentication code and status code as 201
     ...
     #TODO - Deveria retornar 201, mas está retornando status code 200
  Scenario: UC01.14 - The Application must validate if it is a invalid payment QR CODE 03 insufficient balance
